...
// =========================================================================
// SMART TRANSFER: Transfer from Warehouse to Shop (المخزن ➡️ المحل) - ATOMIC BATCH
// =========================================================================
export async function transferFromWarehouseToShop(warehouseProduct, qtyToTransfer, user = null) {
  ...
  try {
    // 1. Fetch fresh stock from Firestore to prevent concurrency or client-side bypass
    ...
    if (existingShopDoc) {
      ...
      batch.set(newShopRef, newShopItem);
    }

    // 3. Log Transfer atomically in nelly_transfers
...
// =========================================================================
// REVERSE TRANSFER: Transfer from Shop to Warehouse (المحل ➡️ المخزن) - ATOMIC BATCH
// =========================================================================
export async function transferFromShopToWarehouse(shopProduct, qtyToTransfer, user = null) {
  ...
  try {
    ...
    if (existingWarehouseDoc) {
      ...
      batch.update(doc(db, WAREHOUSE_COLLECTION, existingWarehouseDoc.id), {
        quantity: currentWhQty + qty,
        updatedAt: new Date().toISOString()
      });
    } else {
      const newWhRef = doc(collection(db, WAREHOUSE_COLLECTION));
      const newWhItem = {
        barcode: barcodeVal || `622${Math.floor(100000 + Math.random() * 900000)}`,
      ...
      batch.set(newWhRef, newWhItem);
    }

    // 3. Log Transfer atomically
    const transferRef = doc(collection(db, TRANSFERS_COLLECTION));
    const transferRecord = {
      from: "المحل",
      to: "المخزن",
