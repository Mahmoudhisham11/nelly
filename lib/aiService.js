...
export function buildStoreContextSnapshot({
  ...
  // Aggregate items across all shift reports + current shift invoices (strictly excluding returned invoices)
  const allInvoices = [
    ...salesInvoices.filter(s => s.status !== "مرتجعة"),
    ...shiftReports.flatMap(r => (r.invoices || []).filter(inv => inv.status !== "مرتجعة"))
  ];
  ...
  // Calculate average daily sales based on closed shifts / distinct local dates
  const shiftDates = shiftReports.map(r => getLocalDateString(r.closedAt || r.date || r.createdAt));
  const uniqueDaysCount = Math.max(1, new Set(shiftDates.filter(Boolean)).size);
