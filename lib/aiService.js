...
export function buildStoreContextSnapshot({
  ...
  expenseItems.forEach(item => {
    const record = monthlyExpensesMap[item.id];
  ...
  return {
    ...
    expenses: {
      ...
      breakdown: currentMonthExpensesBreakdown
    },
...
/**
 * Builds the comprehensive System Instructions Prompt for Minimax on OpenRouter
 */
...
2. عند سؤالك عن النواقص أو البضائع التي قاربت على النفاد، اذكر أسماء الأصناف بدقة مع كمياتها الحالية وأرقام الباركود واقترح الكميات المناسبة لإعادة الطلب.
3. عند سؤالك عن المنتجات الأكثر مبيعاً وضغطاً (Bestsellers)، حدد أسماء الأصناف وعدد القطع المباعة منها وهامش ربحها ووجه صاحب المحل لتكرارها وشراء كميات أكبر منها.
4. عند سؤالك عن الأهداف المالية (مثال: "عايز اجمع 50 ألف جنية" أو أي مبلغ آخر):
...
5. كن احترافياً، مشجعاً، واضحاً، واستخدم تنسيق Markdown المنظم مع جداول وأرقام بارزة بالجنيه المصري (EGP).
