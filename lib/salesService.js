...
export async function returnSaleItemOrInvoice({
  ...
  try {
    ...
    // 5. If from a closed report, update or remove items from the invoice inside the report document
...
export async function searchSoldProductsInReports(searchTerm) {
  ...
  try {
        ...
        items.forEach((item) => {
          ...
          if (matchName || matchBarcode || matchCode || matchId) {
            ...
            if (!productMap.has(groupKey)) {
              // Find matching shop product for live stock
            ...
            const pEntry = productMap.get(groupKey);
    ...
    return Array.from(productMap.values()).sort((a, b) => b.totalSoldQuantityInReports - a.totalSoldQuantityInReports);
  } catch (err) {
...
// Return specific quantity of a product: deduct from nelly_reports and add back to nelly_shop_products
        ...
        for (const item of items) {
          ...
          if (isMatch && remainingToDeduct > 0) {
            ...
            if (currentItemQty <= remainingToDeduct) {
              // Deduct entire item quantity and remove item from invoice
