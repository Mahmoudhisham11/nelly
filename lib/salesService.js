...
import { addInvoiceToCustomerLedger, recordCustomerReturn } from "./customersService";
import { recordSaleCommission, reverseSaleCommission } from "./employeesService";
...
export async function createSaleInvoice(saleData) {
  ...
  try {
    ...
    // 4. If customer is selected, register invoice in customer ledger
    if (customer && customer.id) {
      await addInvoiceToCustomerLedger(customer.id, invoiceNumber, remainingDue, finalTotal);
    }
...
export async function returnSaleItemOrInvoice({
  ...
  try {
    ...
    // 2. Increase stock in shop products (nelly_shop_products) & compute accurate refund with discount
    const stockBatch = writeBatch(db);
    let totalRefundAmount = 0;
    const invoiceSubtotal = Number(invoiceDoc.subtotal) || 0;
    const invoiceDiscount = Number(invoiceDoc.discount) || 0;
    ...
    for (const retItem of itemsToReturn) {
      ...
      const lineRetailTotal = roundCurrency(returnQty * retailPrice);
      // Proportionally deduct discount so store does not over-refund
      const lineRefund = roundCurrency(lineRetailTotal * (1 - discountRatio));
    ...
    // Exact refund check for full invoice return
    const isFullReturn = itemsToReturn.length === (invoiceDoc.items || []).length && itemsToReturn.every(i => {
      const orig = (invoiceDoc.items || []).find(o => 
        (o.productId && o.productId === (i.productId || i.id)) || 
        (o.barcode && o.barcode === i.barcode) || 
        o.name === i.name
      );
      return orig && (parseInt(i.quantity, 10) || 1) >= (orig.quantity || 1);
    });

    if (isFullReturn) {
      totalRefundAmount = roundCurrency(Number(invoiceDoc.paidAmount ?? invoiceDoc.total) || 0);
    } else {
    ...
    // 4. Reversal of Seller Commission if applicable
    if (invoiceDoc.sellerEmployee && invoiceDoc.sellerEmployee.id && totalRefundAmount > 0) {
      await reverseSaleCommission(
        invoiceDoc.sellerEmployee,
        invoiceDoc.invoiceNumber || invoiceNumber,
        returnNumber,
        totalRefundAmount,
        invoiceDoc.sellerEmployee.commissionRate !== undefined ? Number(invoiceDoc.sellerEmployee.commissionRate) : 0.01
      );
    }

    // 5. Update Customer Ledger for return if customer is attached
    if (invoiceDoc.customer && invoiceDoc.customer.id && totalRefundAmount > 0) {
      await recordCustomerReturn(
        invoiceDoc.customer.id,
        invoiceDoc.invoiceNumber || invoiceNumber,
        returnNumber,
        totalRefundAmount,
        Boolean(invoiceDoc.remainingDue && invoiceDoc.remainingDue > 0)
      );
    }

    // 6. Update or Delete from active sales (nelly_sales)
    ...
    // 7. If from a closed report, update or remove items from the invoice inside the report document & sync paymentBreakdown
    if (!isFromActiveSales && foundInReport) {
      const repData = foundInReport.data();
      const repInvoices = repData.invoices || [];
      const updatedRepInvoices = repInvoices.map(inv => {
        if ((inv.invoiceNumber || "").trim().toUpperCase() !== (invoiceDoc.invoiceNumber || "").trim().toUpperCase()) {
          return inv;
        }
        ...
        newSubtotal = roundCurrency(newSubtotal);
        newCost = roundCurrency(newCost);
        const discountVal = roundCurrency(Number(inv.discount) || 0);
        const newTotal = roundCurrency(Math.max(0, newSubtotal - discountVal));
        const newProfit = roundCurrency(newTotal - newCost);
      ...
      let repTotalSales = 0;
      let repTotalCost = 0;
      let repTotalProfit = 0;
      let repTotalItemsSold = 0;
      let repTotalPaid = 0;
      let repTotalDiscount = 0;

      for (const inv of updatedRepInvoices) {
        repTotalSales += (Number(inv.total) || 0);
        repTotalCost += (Number(inv.totalCost) || 0);
        repTotalProfit += (Number(inv.totalProfit) || 0);
        repTotalDiscount += (Number(inv.discount) || 0);
        repTotalPaid += (Number(inv.paidAmount) || 0);
        repTotalItemsSold += (inv.items || []).reduce((acc, curr) => acc + (Number(curr.quantity) || 0), 0);

        const method = inv.paymentMethod || "نقدي";
        const paid = Number(inv.paidAmount ?? inv.total) || 0;
        if (updatedPaymentBreakdown[method] !== undefined) {
          updatedPaymentBreakdown[method] = roundCurrency(updatedPaymentBreakdown[method] + paid);
        } else {
          updatedPaymentBreakdown[method] = roundCurrency(paid);
        }
      }

      await updateDoc(foundInReport.ref, {
        invoices: updatedRepInvoices,
        invoicesCount: updatedRepInvoices.length,
        totalSales: roundCurrency(repTotalSales),
        totalCost: roundCurrency(repTotalCost),
        totalProfit: roundCurrency(repTotalProfit),
        totalDiscount: roundCurrency(repTotalDiscount),
        totalPaid: roundCurrency(repTotalPaid),
        totalItemsSold: repTotalItemsSold,
        paymentBreakdown: Object.fromEntries(
          Object.entries(updatedPaymentBreakdown).map(([k, v]) => [k, roundCurrency(v)])
        ),
        updatedAt: new Date().toISOString()
      });
    }
...
// Search for sold products inside nelly_reports AND active shift nelly_sales, plus live shop stock
export async function searchSoldProductsInReports(searchTerm) {
  if (!searchTerm) return [];
  const cleanTerm = searchTerm.trim().toLowerCase();

  try {
    // 1. Fetch all closed reports from nelly_reports
    const reportsSnap = await getDocs(query(collection(db, REPORTS_COLLECTION), orderBy("closedAt", "desc")));
    
    // 2. Fetch all active shift invoices from nelly_sales
    const salesSnap = await getDocs(query(collection(db, SALES_COLLECTION), orderBy("createdAt", "desc")));

    // 3. Fetch current shop products to get live stock
    const shopSnap = await getDocs(collection(db, SHOP_COLLECTION));
    const shopProducts = shopSnap.docs.map(d => ({ id: d.id, ...d.data() }));

    // 4. Map to aggregate products sold across reports + active shift
    const productMap = new Map();

    const getOrCreateProductEntry = (item, name, barcode, code, productId) => {
      const groupKey = barcode || productId || name;
      if (!productMap.has(groupKey)) {
        const shopItem = shopProducts.find(sp => 
          (barcode && (sp.barcode || "").trim() === barcode) ||
          (productId && sp.id === productId) ||
          (sp.name || "").trim().toLowerCase() === name.toLowerCase()
        );

        productMap.set(groupKey, {
          groupKey,
          productId: productId || (shopItem ? shopItem.id : null),
          shopProductId: shopItem ? shopItem.id : null,
          name: shopItem ? shopItem.name : name,
          barcode: barcode || (shopItem ? shopItem.barcode : ""),
          code: code || (shopItem ? shopItem.code : ""),
          category: shopItem ? shopItem.category : (item.category || "عام"),
          sellingPrice: shopItem ? (shopItem.sellingPrice || item.sellingPrice) : (item.sellingPrice || 0),
          wholesalePrice: shopItem ? (shopItem.wholesalePrice || item.wholesalePrice) : (item.wholesalePrice || 0),
          currentShopStock: shopItem ? (Number(shopItem.quantity) || 0) : 0,
          totalSoldQuantity: 0,
          totalSoldQuantityInReports: 0,
          totalSoldQuantityInActiveShift: 0,
          invoices: []
        });
      }
      return productMap.get(groupKey);
    };

    // A) Process closed reports
    reportsSnap.forEach((repDoc) => {
      const repData = repDoc.data();
      const reportId = repDoc.id;
      const reportNumber = repData.reportNumber || "";
      const closedAt = repData.closedAt || repData.date || "";
      const invoices = (repData.invoices || []).filter(inv => inv.status !== "مرتجعة");
      ...
      invoices.forEach((inv) => {
        ...
        items.forEach((item) => {
          ...
          const matchName = name.toLowerCase().includes(cleanTerm);
          const matchBarcode = barcode.toLowerCase().includes(cleanTerm);
          const matchCode = code.toLowerCase().includes(cleanTerm);
          const matchId = productId.toLowerCase() === cleanTerm;

          if (matchName || matchBarcode || matchCode || matchId) {
            const qty = Number(item.quantity) || 0;
            if (qty <= 0) return;

            const pEntry = getOrCreateProductEntry(item, name, barcode, code, productId);
            pEntry.totalSoldQuantity += qty;
            pEntry.totalSoldQuantityInReports += qty;
            pEntry.invoices.push({
              reportId,
              reportNumber,
              source: "تقرير مؤرشف",
              ...
              cashierName: inv.cashier?.name || "كاشير المحل",
              soldQty: qty,
              sellingPrice: item.sellingPrice || 0,
              subtotal: item.subtotal || qty * (item.sellingPrice || 0)
            });
          }
        });
      });
    ...
    // B) Process active open shift invoices (nelly_sales)
    salesSnap.forEach((saleDoc) => {
      const saleData = saleDoc.data();
      if (saleData.status === "مرتجعة") return;
      const saleId = saleDoc.id;
      const invoiceNumber = saleData.invoiceNumber || "";
      const createdAt = saleData.date || saleData.createdAt || "";
      ...
      items.forEach((item) => {
        ...
        if (matchName || matchBarcode || matchCode || matchId) {
          ...
          const pEntry = getOrCreateProductEntry(item, name, barcode, code, productId);
          pEntry.totalSoldQuantity += qty;
          pEntry.totalSoldQuantityInActiveShift += qty;
          pEntry.invoices.push({
            reportId: null,
            reportNumber: "الوردية الحالية",
            source: "الوردية الحالية المفتوحة",
            isCurrentShift: true,
            activeInvoiceId: saleId,
            closedAt: createdAt,
            invoiceNumber: invoiceNumber,
            customerName: saleData.customer?.name || "عميل نقدي",
            cashierName: saleData.cashier?.name || "كاشير المحل",
            soldQty: qty,
            sellingPrice: item.sellingPrice || 0,
            subtotal: item.subtotal || qty * (item.sellingPrice || 0)
          });
    ...
    return Array.from(productMap.values()).sort((a, b) => b.totalSoldQuantity - a.totalSoldQuantity);
  } catch (err) {
    console.error("searchSoldProductsInReports error:", err);
    throw err;
  }
}

// Return specific quantity of a product: deduct from nelly_sales / nelly_reports and add back to nelly_shop_products
export async function returnProductQuantityFromReports({
  productId,
  shopProductId,
  barcode,
  name,
  ...
  if (isNaN(qtyToReturn) || qtyToReturn <= 0) {
    throw new Error("يرجى إدخال كمية صالحة للإرجاع.");
  }

  try {
    // 1. Update Shop Stock (nelly_shop_products)
    let shopRef = null;
    let shopSnap = null;

    if (shopProductId) {
      shopRef = doc(db, SHOP_COLLECTION, shopProductId);
      shopSnap = await getDoc(shopRef);
    }
    if ((!shopSnap || !shopSnap.exists()) && productId) {
      shopRef = doc(db, SHOP_COLLECTION, productId);
      shopSnap = await getDoc(shopRef);
    }
    if ((!shopSnap || !shopSnap.exists()) && barcode) {
      const bq = query(collection(db, SHOP_COLLECTION), where("barcode", "==", barcode.trim()));
      const bSnap = await getDocs(bq);
      if (!bSnap.empty) {
        shopRef = bSnap.docs[0].ref;
        shopSnap = bSnap.docs[0];
      }
    }
    if ((!shopSnap || !shopSnap.exists()) && name) {
      const nq = query(collection(db, SHOP_COLLECTION), where("name", "==", name.trim()));
      const nSnap = await getDocs(nq);
      if (!nSnap.empty) {
        shopRef = nSnap.docs[0].ref;
        shopSnap = nSnap.docs[0];
      }
    }

    let newShopStock = qtyToReturn;
    if (shopSnap && shopSnap.exists()) {
      const cur = Number(shopSnap.data().quantity) || 0;
      newShopStock = cur + qtyToReturn;
      await updateDoc(shopRef, {
        quantity: newShopStock,
        updatedAt: new Date().toISOString()
      });
    } else {
      // Create product in shop with returned quantity
      ...
      newShopStock = qtyToReturn;
    }

    // 2. Deduct from active shift (nelly_sales) first if items exist
    ...
    for (const saleDoc of salesSnap.docs) {
      ...
      const saleData = saleDoc.data();
      if (saleData.status === "مرتجعة") continue;
      ...
      for (const item of items) {
        ...
        const isMatch = 
          (barcode && itemBarcode === barcode.trim()) ||
          (productId && itemProductId === productId) ||
          (name && itemName.toLowerCase() === name.trim().toLowerCase());

        if (isMatch && remainingToDeduct > 0) {
          saleModified = true;
          const currentItemQty = Number(item.quantity) || 0;

          if (currentItemQty <= remainingToDeduct) {
            remainingToDeduct -= currentItemQty;
          } else {
            const newQty = currentItemQty - remainingToDeduct;
            remainingToDeduct = 0;
            const retail = roundCurrency(Number(item.sellingPrice) || 0);
            const cost = roundCurrency(Number(item.wholesalePrice) || 0);
            updatedItems.push({
              ...item,
              quantity: newQty,
              subtotal: roundCurrency(newQty * retail),
              profit: roundCurrency(newQty * (retail - cost))
            });
          }
        } else {
      ...
      if (saleModified) {
        if (updatedItems.length === 0) {
          ...
          for (const it of updatedItems) {
            newSub += (Number(it.subtotal) || 0);
            newCost += roundCurrency((Number(it.quantity) || 0) * (Number(it.wholesalePrice) || 0));
          }
          newSub = roundCurrency(newSub);
          newCost = roundCurrency(newCost);
          const discountVal = roundCurrency(Number(saleData.discount) || 0);
          const newTot = roundCurrency(Math.max(0, newSub - discountVal));
          const newProf = roundCurrency(newTot - newCost);

          await updateDoc(saleDoc.ref, {
            items: updatedItems,
            itemsCount: updatedItems.reduce((acc, curr) => acc + (Number(curr.quantity) || 0), 0),
            subtotal: newSub,
            totalCost: newCost,
            total: newTot,
            totalProfit: newProf,
    ...
    // 3. Deduct remainder from nelly_reports
    if (remainingToDeduct > 0) {
      ...
      for (const repDoc of reportsSnap.docs) {
        ...
        const repData = repDoc.data();
        const invoices = repData.invoices || [];
        let reportModified = false;

        const updatedInvoices = invoices.map(inv => {
          if (remainingToDeduct <= 0) return inv;
          ...
          if (invModified) {
            ...
            let newCost = 0;
            for (const it of updatedItems) {
              newSubtotal += (Number(it.subtotal) || 0);
              newCost += roundCurrency((Number(it.quantity) || 0) * (Number(it.wholesalePrice) || 0));
            }
            newSubtotal = roundCurrency(newSubtotal);
            newCost = roundCurrency(newCost);
    ...
    console.error("returnProductQuantityFromReports error:", err);
    throw err;
  }
}

// Find all invoices containing a specific product by barcode, code, or name
export async function findInvoicesByProduct(productQuery) {
  if (!productQuery) return [];
  const cleanTerm = productQuery.trim().toLowerCase();

  try {
    const results = [];

    // 1. Check active sales (nelly_sales)
    const salesSnap = await getDocs(query(collection(db, SALES_COLLECTION), orderBy("createdAt", "desc")));
    for (const docSnap of salesSnap.docs) {
      const data = docSnap.data();
      const items = data.items || [];
      const matchingItems = items.filter(it => {
        const nameMatch = (it.name || "").toLowerCase().includes(cleanTerm);
        const barcodeMatch = (it.barcode || "").toLowerCase().includes(cleanTerm);
        const codeMatch = (it.code || "").toLowerCase().includes(cleanTerm);
        const idMatch = (it.productId || it.id || "").toLowerCase() === cleanTerm;
        return nameMatch || barcodeMatch || codeMatch || idMatch;
      });
    ...
    // 2. Check closed reports (nelly_reports)
    const reportsSnap = await getDocs(query(collection(db, REPORTS_COLLECTION), orderBy("closedAt", "desc")));
    for (const repSnap of reportsSnap.docs) {
      const repData = repSnap.data();
      const invoices = repData.invoices || [];
      for (const inv of invoices) {
        ...
        if (matchingItems.length > 0) {
          results.push({
            ...
            sourceLabel: `وردية مقفلة (${repData.reportNumber || ""})`,
            reportId: repSnap.id,
    ...
    console.error("findInvoicesByProduct error:", err);
    return [];
  }
}
